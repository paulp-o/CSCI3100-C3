...
class Login:
    def __init__(self, display, gameStateManager):
        ...
        self.text_password = self.text_font.render(
            'Enter your password', False, 'white')
        self.text_guest = self.text_font.render(
            'Play as a guest', False, 'white')
        self.text_login = self.text_font.render('login', False, 'black')
        self.text_guest_login = self.text_font.render(
            'guest login', False, 'black')
        ...
        self.text_input_id = ''
        self.text_input_pwd = ''
        self.text_input_id_surface = self.text_font.render(
            self.text_input_id, True, (255, 255, 255))
        self.text_input_pwd_surface = self.text_font.render(
            self.text_input_pwd, True, (255, 255, 255))
    ...
    def run(self):
        ...
        while self.loop:
            ...
            pygame.draw.rect(
                Game.screen, self.guest_login_box_color, self.guest_login_box)

            Game.screen.blit(self.text_input_id_surface,
                             (self.id_box.x+5, self.id_box.y+5))
            Game.screen.blit(self.text_input_pwd_surface,
                             (self.pwd_box.x+5, self.pwd_box.y+5))
            ...
            self.id_box.w = max(
                100, self.text_input_id_surface.get_width() + 10)
            self.pwd_box.w = max(
                100, self.text_input_pwd_surface.get_width() + 10)
