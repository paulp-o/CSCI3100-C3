...
class StartGame:
    def __init__(self, display, gameStateManager):
        ...
        # Text
        self.text_title = self.title_font.render(
            'Join / Host Game', False, 'white')
        ...
        self.text_or = self.text_font.render(
            '--------or--------', False, 'white')
        ...
        self.text_input_ip_surface = self.text_font.render(
            self.text_input_ip, True, 'black')
        self.text_input_port_surface = self.text_font.render(
            self.text_input_port, True, 'black')
    ...
    def run(self):
        ...
        Game.screen.blit(self.text_input_ip_surface,
                         (self.ip_box.x+5, self.ip_box.y+5))
        Game.screen.blit(self.text_input_port_surface,
                         (self.port_box.x+5, self.port_box.y+5))
        ...
        while self.loop:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if self.ip_box.collidepoint(event.pos):
                        ...
                        # Now evaluate this result into dictionary
                        result_dict = eval(result)
                        # Now get the player with highest score
                        max_score = 0
                        winner = ''
                        for player in result_dict:
                            if result_dict[player]['score'] > max_score:
            ...
            self.text_input_ip_surface = self.text_font.render(
                self.text_input_ip, True, 'white')
            self.text_input_port_surface = self.text_font.render(
                self.text_input_port, True, 'white')
            Game.screen.blit(self.text_input_ip_surface,
                             (self.ip_box.x+5, self.ip_box.y+5))
            Game.screen.blit(self.text_input_port_surface,
                             (self.port_box.x+5, self.port_box.y+5))
            ...
            self.port_box.w = max(
                100, self.text_input_port_surface.get_width()+10)
